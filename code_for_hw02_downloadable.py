...
def perceptron_origin_averaged(data,labels,T):
    answer = []
    answer_sum = answer
    for _ in range(len(data)):
        answer.append(0)
    ...
    while changed !=False:
        changed = True
        if iteration == T: 
            return answer_sum/no_in_sum # - there is no right answer but this will do...
        for i in range (len(data[0])):
...
def averaged_perceptron(data,labels, params={}, hook=None):
    ...
    origin_answer = perceptron_origin_averaged(new_data,labels,T)
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    ...
    results += th0
    signed_results = np.sign(results)
    #correct = np.sign(labels_test*results.T) #1 if correct
    return (np.sum(signed_results.T == labels_test)/len(data_test[0]))
